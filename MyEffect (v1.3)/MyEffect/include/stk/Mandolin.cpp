...
void Mandolin :: pluck( StkFloat amplitude )
{
  if ( amplitude < 0.0 || amplitude > 1.0 ) {
    oStream_ << "Mandolin::pluck: fAmplitude parameter out of range!";
    handleError( StkError::WARNING ); return;
  }
...
{
  if ( amplitude < 0.0 || amplitude > 1.0 ) {
    oStream_ << "Mandolin::noteOff: fAmplitude is out of range!";
    handleError( StkError::WARNING ); return;
  }

  //strings_[0].setLoopGain( 0.97 + (1 - fAmplitude) * 0.03 );
  //strings_[1].setLoopGain( 0.97 + (1 - fAmplitude) * 0.03 );
}
